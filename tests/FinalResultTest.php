...
final class FinalResultTest extends TestCase
{
    ...
    public function testReturnsTheError(): void
    {
        $finalResult = new FinalResult();
        $result = $finalResult->results('tests/support/notfoundmidori.csv');
        $this->assertEquals($result, [
            "filename" => null,
            "document" => null,
            "failure_code" => 111,
            "failure_message" => 'fopen(tests/support/notfoundmidori.csv): Failed to open stream: No such file or directory',
            "records" => null
        ]);
    }
}
